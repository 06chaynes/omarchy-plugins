...
def parser() -> argparse.ArgumentParser:
    ...
    sync = commands.add_parser("sync"); sync.add_argument("--message", "-m", default=None)
...
def main(plugin_root: Path | None = None, argv=None) -> int:
    ...
    try:
        ...
        if args.command == "sync":
            summary, changes = snapshot(repo, home, apply=True)
            st = repository_status(repo)
            dirty_count = st.get("dirtyFileCount", len(st.get("dirtyFiles", [])))
            commit_res = {}
            if dirty_count > 0 or st.get("ahead", 0) > 0:
                msg = args.message or f"Omarchy configuration sync: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
                if dirty_count > 0:
                    commit_res = commit_snapshot(repo, msg)
                push_res = {}
                if st.get("origin"):
                    push_res = push_snapshot(repo)
                output({
                    "synced": True,
                    ...
                    "commitMessage": commit_res.get("message", msg),
                    "pushed": push_res.get("pushed"),
                    "origin": push_res.get("origin") or st.get("origin"),
                    "repositoryStatus": repository_status(repo)
                })
            else:
                output({
                    "synced": True,
        ...
        if args.command == "snapshot":
            ...
            cats = {
                "themes": [c for c in changes if "themes/" in c],
                "screensavers": [c for c in changes if "screensavers/" in c],
                "terminals": [c for c in changes if any(t in c for t in ["alacritty", "ghostty", "kitty", "foot"])],
                "wm": [c for c in changes if "hypr/" in c and not c.startswith("device/")],
                "scripts": [c for c in changes if ".local/bin/" in c or "hooks/" in c],
                "manifests": [c for c in changes if c.startswith("manifests/")],
                "device": [c for c in changes if c.startswith("device/")],
                "other": [c for c in changes if not any(k in c for k in ["themes/", "screensavers/", "alacritty", "ghostty", "kitty", "foot", "hypr/", ".local/bin/", "hooks/", "manifests/", "device/"])]
            }
            output({
                ...
                "categoryDetails": {k: v for k, v in cats.items() if v},
                "repositoryStatus": repository_status(repo),
                "next": "Review with git diff; Synchro never commits or pushes."
            })
            return 0
