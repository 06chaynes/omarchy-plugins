...
/// Detect if the image has a dark background (mean luminance < 0.5)
fn is_dark_image(src: &str) -> bool {
    let magick = match which("magick") {
        Ok(p) => p,
        Err(_) => return false,
    };
    let output = match run_command(
        &magick,
        &[src, "-colorspace", "Gray", "-format", "%[fx:mean]", "info:"],
        None,
        SHORT_PROCESS_TIMEOUT,
        64,
        MAX_COMMAND_STDERR_BYTES,
    ...
    if let Ok(mean) = s.trim().parse::<f64>() {
        mean < 0.5
    } else {
        false
    }
...
/// Pre-process the image with ImageMagick to improve OCR accuracy.
///
/// 1. Grayscale conversion and dark-background auto-inversion (Tesseract neural net is
///    trained predominantly on dark-on-light text).
/// 2. Auto-leveling and contrast normalization to eliminate background blur noise.
/// 3. Adaptive 300% upscale with sharpening for small/medium screen regions.
fn preprocess(src: &str) -> Result<TempFile, String> {
    ...
    // Invert dark background so text is black on white (huge boost for dark mode apps)
    if is_dark {
        args.push("-negate".into());
    }

    // Dynamic contrast leveling
    args.push("-auto-level".into());
    args.push("-contrast-stretch".into());
    args.push("0.1%x0.1%".into());

    // Upscale only when capture is small/medium (region grabs, low-res fonts)
    if w > 0 && h > 0 && w < 1600 && h < 1600 {
        args.push("-resize".into());
        args.push("300%".into());
    }
    args.push("-sharpen".into());
    args.push("0x0.75".into());
...
fn ocr_single_mode(path: &str, lang: &str, psm: &str) -> Result<(String, f64), String> {
    let tesseract = which("tesseract")?;

    // 1. Text extraction with dictionary bypass for code/symbols and 300 DPI hint
    let output = run_command(
        &tesseract,
        &[
            path,
    ...
    if tsv_output.status.success() {
        if let Ok(tsv_text) = String::from_utf8(tsv_output.stdout) {
            let mut sum = 0.0;
            let mut n = 0u32;
...
/// Adaptive OCR: tries PSM 6 (uniform block), and falls back to PSM 11 (sparse text) or
/// PSM 3 (auto) if confidence is low or text is empty.
fn ocr_image_adaptive(path: &str, lang: &str) -> Result<(String, f64), String> {
    let (text_6, conf_6) = ocr_single_mode(path, lang, "6")?;
    if conf_6 >= 75.0 && !text_6.trim().is_empty() {
        return Ok((text_6, conf_6));
    }

    // Try sparse text mode (great for UI chips, single lines, scattered text)
    let (text_11, conf_11) = ocr_single_mode(path, lang, "11").unwrap_or_default();
    if conf_11 > conf_6 && !text_11.trim().is_empty() {
        return Ok((text_11, conf_11));
    }

    // If both were low or empty, try standard auto (PSM 3)
    if conf_6 < 50.0 && conf_11 < 50.0 {
        if let Ok((text_3, conf_3)) = ocr_single_mode(path, lang, "3") {
            if conf_3 > conf_6 && conf_3 > conf_11 && !text_3.trim().is_empty() {
                return Ok((text_3, conf_3));
            }
        }
    }
...
pub fn run_ocr(opts: OcrOptions) -> Result<OcrResult, String> {
    ...
    // 3. OCR with adaptive PSM and confidence selection
    let (text, raw_conf) = ocr_image_adaptive(&ocr_input, &lang)?;
    let confidence = if raw_conf.is_finite() {
        raw_conf.clamp(0.0, 100.0)
    } else {
        0.0
    };
